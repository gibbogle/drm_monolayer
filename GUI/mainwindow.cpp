...
//        if (p.tag.contains("SAVE_SLICE_DATA_NUMBER")) {   // insert the drug data here, before plot data
        if (p.tag.contains("SPCRAD")) {   // insert the drug data here, before plot data
            ndrugs = 0;
            if (ProtocolUsesDrug()) {
...
void MainWindow::showSummary(double hr)
{
    ...
    LOG_QMSG("showSummary: " + QString::number(hr));
    step++;
    LOG_QMSG("step: " + QString::number(step));
    if (step > newR->nsteps) {
    ...
    // TS plots
    for (int i=0; i<nGraphs; i++) {
//       LOG_QMSG("TS plot: i: " + QString::number(i));
        if (!grph->isTimeseries(i)) continue;
        if (!grph->isActive(i)) continue;
        int k = grph->get_dataIndex(i);
//        LOG_QMSG("summary index: k: " + QString::number(k));
        val = Global::summaryData[k];
//        LOG_QMSG("summary: val: " + QString::number(val));
...
//            }
            ...
            nc = Global::conc_nc_ex;
            dx = Global:: conc_dx_ex;
            LOG_QMSG("Profile plot: tag: " + tag + "  k: " + QString:: number(k) + "  nc: " + QString::number(nc));
            if (k == MULTI) {
                if (IC) {
...
// 8 DrugB
// 9 DrugB metabolite 1
//10 DrugB metabolite 2
// ...
//
...
void MainWindow::setupConstituents(int ndisplay)
{
    ...
    for (ivar=0; ivar<Global::nvars_used; ivar++) {
        ...
        LOG_QMSG(Global::var_string[ivar]);
    }
    free(name_array);
