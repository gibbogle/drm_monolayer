            ...
            if (i > 0) {
                ...
                if (!ok1) {
                    QMessageBox::warning(this, tr("Application"),
                                         tr("Drug file %1:\n%2.")
                                         .arg(fileName)
                                         .arg(" has the wrong number of parameters.  Check the file format. (1)"));
                    return;
                }
            }
            if (i==0) {
...
//                                         tr("Drug file %1:\n%2.")
//                                         .arg(fileName)
//                                         .arg(" has the wrong number of parameters.  Check the file format. (2)"));
//                    return;
//                }
...
void MainWindow::writeDrugParams(QTextStream *out, int idrug)
{
    ...
    line += "CLASS_NAME";
    *out << line + "\n";
    int nmet = drug[idrug].nmetabolites;
...
void MainWindow::readDrugData(QTextStream *in)
{
    ...
    for (int idrug=0; idrug<ndrugs; idrug++) {
        ...
        line = in->readLine();  // N_METABOLITES
        data = line.split(" ",QString::SkipEmptyParts);
        int nmet = data[0].toInt();
...
//                text_DRUG_B_NAME->setText(drugname);
        ...
        // Now skip the rest of the drug section (drug data has already been loaded)
        int nskip = (nmet+1)*(6 + 2*(NDKILLPARAMS + NIKILLPARAMS)) - 1;
        LOG_QMSG("nmet: " + QString::number(nmet));
        LOG_QMSG("nskip: " + QString::number(nskip));
        for (int k=0; k<nskip; k++) {
            line = in->readLine();
        }
    }
    LOG_QMSG("did readDrugData");
}
