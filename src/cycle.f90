...
real(REAL_KIND) :: cf_O2, cf_glucose, pcp_O2, pcp_glucose, pcp_starvation, R, fV, tfactor
...
!        if (kcell_now == 40) write(*,'(a,i6,2f8.3)') 'log_timestep:  S: kcell,tfactor: ',kcell_now,tfactor,ccp%T_S/3600
...
!	endif
    if (cp%S_flag .and. cp%SG2_flag) then
        cp%phase = G2_phase
        tfactor = (max_growthrate(ityp)/cp%dVdt)*cp%fg
!        if (kcell_now == 40) write(*,'(a,i6,2f8.3)') 'log_timestep: G2: kcell,tfactor: ',kcell_now,tfactor,ccp%T_G2/3600
