...
!	real(REAL_KIND), allocatable :: Fcurr_b(:,:,:)
	...
	real(REAL_KIND) :: Cmedium(N1D)     ! concentrations in the medium - depth profile
end type
...
real(REAL_KIND) :: C_OGL(4,N1D)			! 1D medium concentrations (for OGL)
...
do ichemo = 1,MAX_CHEMO
	...
	if (chemo(ichemo)%used) then
		if (ichemo >= OXYGEN .and. ichemo <= GLUCOSE) then
